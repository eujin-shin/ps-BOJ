...
object Implementation extends Template {
  ...
  */
  def pdafs2es(pda: PDA): PDA = 
    ...
    val tmp = 
    Map((is, None, ia) -> Set((stateMap(pda.initState), List(alphabetMap(pda.initAlphabet), ia)))) 
    ++ (pda.trans.map{case ((p, opt, x), set) => 
     ((stateMap(p), opt, alphabetMap(x)), (transSetMap(set)))
    }.toMap) 
    ++ (for{
      pfs <- pda.finalStates; 
      x <- alphabets
     } yield (stateMap(pfs), None, x) -> Set((fs, List[Alphabet]()))).toMap
    ++ (for{
      x <- alphabets
    ...
    val trans5: Map[(State, Option[Symbol], Alphabet), Set[(State, List[Alphabet])]] = (for{
      p <- states;
      a <- pda.symbols;
      x <- alphabets
    } yield (p, Some(a), x) -> tmp.getOrElse((p, Some(a), x), Set())).toMap

    val trans: Map[(State, Option[Symbol], Alphabet), Set[(State, List[Alphabet])]] = (for{
      p <- states;
      a <- pda.symbols;
      x <- alphabets
    } yield (p, Some(a), x) -> tmp.getOrElse((p, Some(a), x), Set())).toMap
    ++ (for{
      p <- states;
      x <- alphabets
    } yield (p, None, x) -> tmp.getOrElse((p, None, x), Set())).toMap
